...
These data types are export-only:

- **Contacts & Users** - Portal users and company contacts
- **Meetings** - Meeting records with notes and schedule
- **Meeting Action Items** - Follow-up tasks from meetings
- **Meeting Notes** - Per-user notes attached to meetings
- **Meeting Agenda Items** - Agenda entries for meetings
- **Budgets** - Budgets with one row per budget line item
- **Budget Products** - Your budgeting product catalog
- **Survey Responses** - CSAT, NPS, and general survey responses
- **Training Records** - Training enrollments with progress and certificates
...
- **Onboarding Tasks** - Client onboarding tasks with status and completion dates
- **Broadcasts** - Broadcast messages with content and delivery counts
- **AI Assistant Conversations** - AI assistant chat history

Exports only include data created and managed in MSPortal. Data synced from your PSA (tickets, invoices) is not included: your PSA remains the system of record.
...
Click **Export All (ZIP)** to download every data type at once as a single ZIP archive of CSV files (named `msportal_export_{company}_YYYYMMDD.zip`). Use this for backups, compliance evidence, or a complete copy of your data when offboarding.
