...
Ticket Display Groups can now scope a role to specific PSA service boards or queues, so client-portal users only ever see tickets from the boards you choose for them. This extends the existing ticket visibility model (all, own, location) with a board-level dimension.

**Key Features:**

- **Board and queue scoping**: In a Display Group, pick the allowed service boards or queues from a dropdown populated by your synced PSA boards. No board IDs to type, no guesswork.
- **Enforced everywhere**: The restriction applies to the ticket list, Ticket Trends drill-downs, and dashboard ticket counts and charts, so totals never expose boards a user cannot open.
- **Opt-in and permissive by default**: A Display Group with no boards selected stays unrestricted. When a user has more than one role, they see the union of all allowed boards.
- **Client users only**: Tenant and staff users are never board-restricted.
