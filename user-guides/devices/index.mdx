...
The Devices module provides a centralized inventory of all IT assets across your managed companies. Track devices, view detailed hardware and security information, manage warranty data, and integrate with tools like Addigy, Datto RMM, NinjaOne, Syncro, Asio, and Cisco Meraki.
...
<CardGroup cols={2}>
  ...
  <Card title="Approximate IP Location" icon="location-dot" href="/user-guides/devices/ip-location">
    Use an integration-reported external IP to estimate where a device connects to the internet.
  </Card>
</CardGroup>
...
Click any device row to open the **Device Detail** panel. This provides a comprehensive view of the device with extended data pulled from your RMM integration.

The detail panel includes the following sections (sections only appear when data is available from your integration):

| Section | Information Shown |
|---------|-------------------|
| **Overview** | OS, model, manufacturer, serial number, IP address, MAC address |
| **IP location** | External IP, observation time, approximate location, accuracy radius, and map link |
| **Hardware** | Processor, RAM, CPU cores, total and free disk space with usage bar |
| **Security** | FileVault, Firewall, Gatekeeper, SIP, MDM enrollment, antivirus, patch status |
| **Network** | Hostname, domain, LAN IP, WAN IP, Wi-Fi and Ethernet MAC addresses |
| **Battery** | Battery percentage with visual indicator, charging status, cycle count |
| **User** | Current logged-in user, user email, last logged-in user |
| **System** | Uptime, last boot time, OS build, architecture, agent version, pending reboot |
| **Provider** | Integration-specific data and remote access links (TeamViewer, ScreenConnect, Splashtop) |
| **Warranty** | Warranty start/end dates with active or expired badge |
...
| **Addigy** | Apple devices synced from Addigy MDM |
| **Datto RMM** | Devices synced from Datto RMM |
| **NinjaOne** | Devices synced from NinjaOne |
| **ConnectWise** | Configurations synced from ConnectWise Manage |
| **Halo** | Assets synced from Halo PSA |
| **Syncro** | Devices synced from Syncro RMM integration |
| **Asio** | Devices synced from Asio network discovery |
| **Liongard** | Systems synced from Liongard |
...
- [Find a Device by Its Approximate IP Location](/user-guides/devices/ip-location) - Look up a device's external IP and open the approximate result on a map
- [Understand Device IP Location Results](/user-guides/devices/ip-location-accuracy-and-privacy) - Learn about accuracy, caching, privacy, and common limitations
- [Import Devices](/user-guides/integrations/import-devices) - Device type mappings, auto-sync, and import workflow
- [Addigy Integration](/user-guides/integrations/enable-addigy-integration) - Apple device management integration
- [Datto RMM Integration](/user-guides/integrations/enable-datto-rmm-integration) - Datto RMM device sync
- [Liongard Integration](/user-guides/integrations/enable-liongard-integration) - Configuration audit integration with device tab
