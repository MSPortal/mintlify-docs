...
Use the **View as** dropdown to open the client portal exactly as a specific contact sees it. It appears in three places:

- The portal's **Overview** tab
- The portal's **Pages** tab
- The portal list under **Settings > Onboarding**

Pick a contact from the dropdown, and the portal opens in a new tab in that contact's session — same branding, same visible tasks, same pages. If the portal has no contacts yet, preview by **role** instead.
...
<Note>
Preview sessions are **read-only**. The portal shows a "Preview mode" banner and nothing you enter is saved, so you can click through tasks and forms freely without recording progress or triggering notifications. Preview sessions last 24 hours and require onboarding write permission.
</Note>
...
<Info>
When **Status sync** is enabled, MSPortal refreshes linked onboarding ticket statuses from your PSA every 30 minutes.
</Info>
