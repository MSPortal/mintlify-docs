...
- **Search** - Find projects by project name, company name, or outcome title
...
<AccordionGroup>
  ...
  <Accordion title="Group By Options">
    ...
    - **Status** (default) - Group by ticket status. Dragging a card between columns updates the ticket status in ConnectWise or the task status in Autotask.
    - **Technician** - Group by assigned owner. By default only technicians who already own tickets get a column, plus an **Assign to…** column that opens a technician picker for anyone else in the roster. Administrators can switch to one column per assignable technician and limit the roster by PSA role, department, team, or line of business under **Settings > Projects > Kanban technicians**.
