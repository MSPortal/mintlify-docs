...
<Steps>
  ...
  <Step title="Start Setup">
    Find the **Syncro** integration card and click **Setup** (or **Configure** if previously connected) to launch the setup wizard.
  </Step>
</Steps>
...
MSPortal checks what your Syncro API token is allowed to read, and shows the result for each area of data.

<Steps>
  <Step title="Review the results">
    The check runs automatically when you reach this step. Each area of data is listed as either granted or unavailable.

    Areas marked **Required** must be granted before you can continue: Companies, Contacts and staff, Tickets, and Boards and statuses (read from ticket settings). Assets are optional.
  </Step>

  <Step title="Grant anything missing">
    Any area that is not permitted shows the exact token permission to enable, for example **Assets - List/Search**.

    Edit the MSPortal token under **Admin > API Tokens**, enable the permission, then come back and click **Re-check**.

    <Note>
      Syncro returns the same error for an out-of-scope token as it does for an invalid one. If everything comes back unavailable, confirm the token itself is still valid before adjusting individual permissions.
    </Note>
  </Step>
...
<Steps>
  ...
  <Step title="Configure Contact Auto-Import">
    ...
    <Note>
      A toggle appears locked when the permission check found that your API token cannot read that data. Enable the permission named beside it in Syncro, then re-run the check on the Permissions step to unlock it.
    </Note>
...
Import users, companies, and devices from Syncro. This step is optional - you can skip it and import later from the respective settings pages. The step is organized into **Users**, **Companies**, and **Devices** tabs.
...
<Tabs>
  ...
  <Tab title="Companies">
    ...
    <Tip>
      Companies will sync automatically based on your sync settings. Use this tab if you want to import specific companies immediately.
    </Tip>
  </Tab>

  <Tab title="Devices">
    Once your Syncro integration is connected, you can import devices/assets.

    1. Click **Open Device Import** to open the full device import page
    2. Map Syncro asset types to MSPortal device types and select devices to import

    <Note>
      You can also import devices later from the integration settings.
    </Note>
  </Tab>
</Tabs>
...
| Tickets not syncing | Verify **Enable Ticket Sync** is turned on in the Sync step |
