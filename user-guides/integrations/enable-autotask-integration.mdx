...
<Steps>
  <Step title="Review the results">
    ...
    Areas marked **Required** must be granted before you can continue: Companies, Contacts and staff, Tickets, and Boards and statuses (the ticket picklists). Without these the integration cannot function.

    Optional areas cover Configurations, Opportunities, Product catalog, Invoices, Contracts, Projects, Time entries, Service calls, and Company notes.
  </Step>

  <Step title="Grant anything missing">
    Any area that is not permitted shows the exact security level setting to change, for example **Security Level -> Contracts -> Products & Services: View**.

    Update the security level assigned to your API user in **Admin > Resources/Users > Security Levels**, then come back and click **Re-check**.
  </Step>
...
<Steps>
  ...
  <Note>
    A toggle appears locked when the permission check found that your API user cannot read that data. Grant the permission named beside it in Autotask, then re-run the check on the Permissions step to unlock it.
  </Note>
...
<Tabs>
  ...
  <Tab title="Configuration Items">
    Import Autotask configuration items as devices in MSPortal. Map configuration item types to MSPortal device types and select the items to import.
  </Tab>
</Tabs>
...
| Webhooks not working | Ensure your API user's security role can create webhooks, then re-run the Connect step of the setup wizard so webhooks are re-provisioned automatically. |
