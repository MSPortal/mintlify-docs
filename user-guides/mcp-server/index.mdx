...
- An MCP-compatible AI client (Claude, ChatGPT/Codex, Cursor, GitHub Copilot, VS Code, Gemini CLI, Windsurf, or any client that supports remote MCP servers).
...
<Tabs>
  ...
  <Tab title="GitHub Copilot (VS Code)">
    1. Click **Add to GitHub Copilot (VS Code)** (one-click install), or run the provided `code --add-mcp` command.
    2. When prompted, allow VS Code to start the server and sign in.
    3. Approve access in the browser window that opens.
    4. Open GitHub Copilot Chat in **Agent** mode and confirm the MSPortal tools are enabled under **Configure Tools**.

    See [Connect GitHub Copilot](/user-guides/mcp-server/github-copilot) for the complete VS Code and Copilot CLI guide.
  </Tab>
  <Tab title="GitHub Copilot CLI">
    1. Run this command:
       ...
       ```
    2. Start Copilot CLI and run `/mcp auth msportal`.
    3. Approve access in the browser window that opens.

    See [Connect GitHub Copilot](/user-guides/mcp-server/github-copilot) for manual configuration and troubleshooting.
  </Tab>
...
<Tip>
Tools that show a **One-click install** badge (Cursor and GitHub Copilot in VS Code) can be set up with a single button. For everything else, copy the provided command or config; the server URL is already filled in.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Compliance" icon="clipboard-check">
    Review portfolio compliance, manage assigned checks and runs, maintain groups, check definitions, and reusable run templates, and turn failed checks into tracked risks.
  </Card>
  ...
  <Card title="Standards" icon="list-ol">
    Read and maintain your technology standards catalog, review per-company deployment status, and use compliance rollups to find clients off-standard.
  </Card>
  ...
  <Card title="Backup" icon="database">
    Read unified backup summaries, protected devices, and alerts across Cove, Acronis, and NinjaOne, plus Kaseya SaaS Protection customer health and seat inventory.
  </Card>
...
- [Connect GitHub Copilot](/user-guides/mcp-server/github-copilot) — add MSPortal to Copilot Chat in VS Code or to Copilot CLI.
