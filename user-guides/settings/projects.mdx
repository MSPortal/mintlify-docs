---
title: 'Project Settings'
description: 'Configure project display options, status mappings, and feature visibility'
icon: 'folder-gear'
---

The Project Settings page allows you to customize how projects are displayed throughout MSPortal. Control which projects client users can access, which information appears on project cards, configure status column mappings for kanban boards, and enable or disable project features.

Navigate to **Settings > Projects** to access these options.

## Client Project Access

The **Client project access** section controls which projects your client (company) users can see in the Projects module. These settings only affect client users; your internal team continues to see all projects.

Choose one of two modes:

| Mode | Behavior |
|------|----------|
| **All projects** | Show every project available for each company. This is the default. |
| **Filtered projects** | Only show projects matching the selections below. |

### Filtering Projects

When you select **Filtered projects**, the filters below appear. A project is shown when it matches **at least one** selected type, line of business, board, or specific project.

| Filter | What it includes | Available for |
|--------|------------------|---------------|
| **Project types** | Every project with a selected PSA project type | All PSAs |
| **Lines of business** | Every project assigned to a selected Autotask line of business (Organizational Structure) | Autotask |
| **Project boards** | Projects with phases or tickets on a selected project board | ConnectWise, Halo |
| **Specific projects** | Hand-picked projects, chosen with a searchable selector | All PSAs |

<Note>
Autotask does not have project boards, so Autotask tenants see the **Lines of business** filter instead. Lines of business are synced from Autotask's Organizational Structure on each project sync.
</Note>

<Steps>
  <Step title="Select Filtered Projects">
    In the **Client project access** section, choose **Filtered projects**.
  </Step>
  <Step title="Choose Your Filters">
    Select PSA project types, lines of business or project boards, or search for specific projects. You can combine any of the filters.
  </Step>
  <Step title="Save">
    Save your settings. Client users immediately see only the projects that match your selections.
  </Step>
</Steps>

<Warning>
If you select **Filtered projects** without choosing any filters, client users will not see any projects. A warning appears in the settings page when no filters are selected.
</Warning>

<Note>
Projects explicitly hidden on their own project page remain hidden, even when they match these filters. Hiding a project individually always takes priority.
</Note>

Global search results follow the same access rules, so client users never see a project in search that they can't open.

## Section Visibility

Control which major sections appear in the Projects module by toggling these options:

| Section | Description |
|---------|-------------|
| **Timeline** | Show project timeline with start and due dates |
| **Progress** | Show progress bars and completion percentage |
| **Hours** | Show estimated and actual hours |
| **Financials** | Show revenue and budget information |
| **Details** | Show project summary and description |
| **PSA Phases** | Show phases synced from your PSA system |
| **Project Tickets** | Show tickets under phases (requires PSA Phases) |
| **Time Entries** | Show time entries under tickets (requires Tickets) |
| **Project Summary** | Show the PSA project summary card on the project detail page |

<Note>
Some sections have dependencies. For example, **Project Tickets** requires **PSA Phases** to be enabled, and **Time Entries** requires **Project Tickets** to be enabled.
</Note>

### Kanban Settings

When the Kanban board is enabled, you can configure:

- **Show Kanban** - Enable/disable the Kanban tab in project details
- **Group By** - Default grouping for ticket columns (Status, Technician, or Priority)
- **Density** - Default card density (Comfortable, Compact, or Dense)

### Kanban Technicians

The **Kanban technicians** card controls which PSA agents appear as technician columns and can be assigned tickets when the board is grouped by technician.

| Setting | Options |
|---------|---------|
| **Technician columns** | **Only technicians with tickets** (default) shows an Unassigned column plus one column per technician who owns a ticket on the project, and an **Assign to…** drop target for everyone else in the roster. **Everyone in the roster** shows a column for every assignable technician. |
| **PSA groups** | Roles, departments, teams, and lines of business synced from your PSA. A technician is assignable when they belong to any selected group. |
| **Specific technicians** | Agents that are always assignable, regardless of group membership. |

Leave both roster fields empty to allow every active PSA agent. ConnectWise projects are always limited to the project team in ConnectWise; the roster narrows that team further when configured.

<Note>
Groups come from your PSA: Autotask roles, departments and lines of business (Organizational Structure), ConnectWise departments, work roles and security roles, and Halo teams. They appear after the next PSA user sync.
</Note>

## Field Visibility

Fine-tune which fields appear on project cards with these toggles:

<AccordionGroup>
  <Accordion title="Basic Information">
    - **Project Name** - The project title
    - **Company** - The client company name
    - **Status** - Project lifecycle status badge
    - **Summary** - Project description text
    - **Project Lead** - Assigned project manager
  </Accordion>
  <Accordion title="Progress Fields">
    - **Progress %** - Completion percentage
    - **Estimated Hours** - Budgeted hours
    - **Actual Hours** - Hours worked
    - **Hours Variance** - Difference between estimated and actual (under/over)
  </Accordion>
  <Accordion title="Timeline Fields">
    - **Start Date** - Project start date
    - **Due Date** - Project deadline
    - **Timeline Comparison** - Visual progress indicator on timeline
    - **Last Changed** - When the project was last updated
  </Accordion>
  <Accordion title="Financial Fields">
    - **Estimated Revenue** - Projected revenue
    - **Actual Revenue** - Revenue earned
    - **Revenue Variance** - Difference between estimated and actual
  </Accordion>
</AccordionGroup>

### Live Preview

As you toggle field visibility options, the **Live Preview** card on the right side of the page updates in real-time. This shows you exactly how project cards will appear with your current settings.

## Status Mapping

MSPortal uses status mappings to organize projects into kanban columns. There are two separate mapping configurations:

### Project Kanban Columns

This mapping controls how **project lifecycle statuses** (from your PSA) map to display columns on the main Projects dashboard kanban.

**Default Column Groups:**
- Not Started
- In Progress
- On Hold
- Completed
- Cancelled

<Steps>
  <Step title="View Current Mappings">
    The **Project Kanban Columns** section shows each column and which PSA statuses are assigned to it.
  </Step>
  <Step title="Add Statuses to a Column">
    Click on a column to expand it. Use the dropdown to select PSA statuses that should appear in that column.
  </Step>
  <Step title="Set Default Status">
    For each column, you can set a **default status**. When you drag a project to that column and only one status is available, this status will be applied automatically.
  </Step>
  <Step title="Remove Statuses">
    Click the **X** next to any status to remove it from the column mapping.
  </Step>
</Steps>

<Tip>
You can add the same status to only one column. If a status is already mapped to another column, it won't appear in the dropdown for other columns.
</Tip>

### Ticket Kanban Columns

This mapping controls how **ticket statuses** (from your PSA service boards) map to columns in the project-level ticket kanban view.

<Note>
This is separate from the project status mapping because tickets use different status values than projects. For example, ticket statuses might include "New," "Scheduled," "In Progress," "Waiting Customer," while project statuses might be "Planning," "Active," "On Hold."
</Note>

The configuration works the same as project status mapping:

1. Each column can have multiple ticket statuses mapped to it
2. Set a default status for auto-selection when dragging
3. Unmapped statuses can be added dynamically when moving tickets

## Managing Status Mappings

### Adding New Statuses

When a new status appears in your PSA that isn't mapped to any column:

1. The status will appear in the **Unmapped Statuses** list
2. Drag or select it to add it to the appropriate column
3. Save your changes

### Auto-Add Unmapped Statuses

When you drag a project or ticket to a column that doesn't have the required status mapped:

1. MSPortal detects the unmapped status
2. If you have settings permissions, you'll be prompted to add the status to the column
3. Choose **Yes** to add it automatically, or **No** to just move the item this once

This helps keep your mappings up-to-date as new statuses are created in your PSA.

## Resetting to Defaults

Click the **Reset to Defaults** button to restore all project display settings to their original values. This will:

- Set client project access back to **All projects** and clear any filters
- Re-enable all section and field visibility toggles
- Reset status mappings to default configurations
- Reset kanban settings to defaults

<Warning>
Resetting to defaults cannot be undone. Make sure you want to clear all customizations before clicking this button.
</Warning>

## Project Templates

The **Templates** tab holds reusable project blueprints: phases, tickets with hours and due-date offsets, and the client decisions and action items each project needs. A template can be turned into a project in your PSA or as an MSPortal project from **Projects > New project**, or applied to an existing project.

### Building a template

Click **New template** to open the builder. Add phases, tickets, decisions, and action items by hand, or open the **AI Template Designer** from the button on the right edge of the page. Describe the project in plain language and the designer drafts the full structure; review each suggested change, accept or reject it, and apply. Applied changes can be undone from the sidebar.

### Importing templates from your PSA

Click **Import from PSA** on the Templates tab to bring existing templates into MSPortal.

| Source | Available for |
|--------|---------------|
| **PSA templates** | ConnectWise project templates and Halo PSA project templates |
| **Existing project** | Any synced project from ConnectWise, Autotask, or Halo PSA |

Autotask does not expose project templates through its API, so Autotask users build templates from an existing project instead. Imported templates stay linked to their source and show the provider next to their name.

ConnectWise requires the MSPortal security role to include **Project > Project Templates** with **Inquire Level: All**. If the dialog reports that ConnectWise refused the request, add that permission under **System > Security Roles** in ConnectWise and try again.

### Updating an imported template

Choose **Update from ConnectWise**, **Update from Halo PSA**, or **Update from Synced project** from a template's row menu, or from the button in the template builder, to re-read the source. Phases and tickets are replaced with the current structure from the source. Decisions, action items, and ticket assignees that were added in MSPortal are kept.

## Permissions

| Permission | Capability |
|------------|------------|
| `read_settings_projects` | View project display settings |
| `write_settings_projects` | Modify project display settings |
| `manage_settings_projects` | Full access including status mapping |

## Best Practices

<CardGroup cols={2}>
  <Card title="Start Simple" icon="seedling">
    Begin with default settings and customize as you learn what information your team needs most.
  </Card>
  <Card title="Match Your Workflow" icon="diagram-project">
    Map PSA statuses to columns that match how your team actually works and thinks about project stages.
  </Card>
  <Card title="Set Defaults" icon="bullseye">
    Configure default statuses for each column to speed up drag-and-drop operations.
  </Card>
  <Card title="Keep It Clean" icon="broom">
    Hide fields that your team doesn't use to reduce visual clutter on project cards.
  </Card>
</CardGroup>

## Related Resources

<CardGroup cols={2}>
  <Card title="Projects Overview" icon="folder-open" href="/user-guides/projects/index">
    Learn how to use the Projects module
  </Card>
  <Card title="Integrations" icon="plug" href="/user-guides/integrations/index">
    Set up your PSA integration to sync projects
  </Card>
</CardGroup>
